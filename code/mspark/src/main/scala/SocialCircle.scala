...
object SocialCircle {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setMaster("local[20]").setAppName("SocialCircle")
    val sc = new SparkContext(conf)
    //val rdd = sc.textFile("data/social_circle_uniq_sample.txt").filter(!_.isEmpty).filter(!_.startsWith("#")).map(line => {
    val rdd = sc.textFile("data/social_circle_uniq.data.gz").filter(!_.isEmpty).map(line => {
      val tmp = line.split(" ")
